...
class Ciena8190:
    ...
    def create_flow_point(
        ...
        return Ciena8190.CONFIG.format(
            config=Ciena8190.CREATE_FLOW_POINT.format(
                ...
                fd=fd,
                port=port,
                classifier=classifier,operation=operation))
    ...
    def delete_classifier(
        ...
        """
        return Ciena8190.CONFIG.format(
            config=Ciena8190.DELETE_CLASSIFIER.format(
                name=name))
    ...
    def delete_forwarding_domain(
        ...
        """
        return Ciena8190.CONFIG.format(
            config=Ciena8190.DELETE_FORWADING_DOMAIN.format(
                name=name))
    ...
    def delete_flow_point(
        ...
        """
        return Ciena8190.CONFIG.format(
            config=Ciena8190.DELETE_FLOW_POINT.format(
                name=name))
    ...
    def disable_flow_point(
        ...
        """
        return Ciena8190.CONFIG.format(
            config=Ciena8190.DISABLE_FLOW_POINT.format(
                name=name))
...
class Backend(BaseBackend):
    ...
    def __init__(self):
        ...
        self._settings = BackendSettings(_env_file=(env_file := find_file("ciena8190.env")))
        ...
        if not hasattr(self, '_ports'):
            ...
            for stp in self.topology():
                ...
                for vlans in stp.vlans.split(','):
                    (start_vlan, end_vlan) = (int(vlans), int(vlans)) \
                        if '-' not in vlans \
    ...
    def _get_stp_id(
        ...
        for (start_vlan, end_vlan), stp_id in ports[port_id].items():
            if start_vlan <= vlan <= end_vlan:
                return stp_id
        raise NsiException(GenericRmError, 'VLAN not found on the port', port_id=port_id, vlan=vlan)

    def _get_manager(self) -> manager.Manager:
        """Return an active NETCONF manager session"""
        if not self._manager \
        or not self._manager.connected:
            try:
            ...
            self.log.info('Connected to device', host=self._settings.host, port=self._settings.port, username=self._settings.username)
    ...
    def _create_classifier(
        ...
        """Create a classifier on the device"""
        self.log.info(
            'Creating source classifier',
            name=name,
            vlan=vlan)
        try:
            self._get_manager().edit_config(
                target=self._get_target(),
                config=Ciena8190.create_classifier(
                    name=name,
                    vlan=vlan))
        except Exception as e:
        ...
        self.log.info(
            'Source classifier created successfully',
            name=name,
            vlan=vlan)
    ...
    def _create_forwarding_domain(
        ...
        self.log.info(
            'Forwarding domain created successfully',
            name=name)
    ...
    def _delete_classifier(self, name: str) -> None:
        ...
        try:
            self._get_manager().edit_config(
                target=self._get_target(),
                config=Ciena8190.delete_classifier(name=name))
        except Exception as e:
    ...
    def _delete_forwarding_domain(self, name: str) -> None:
        ...
        self.log.info(
            'Forwarding domain deleted successfully',
            name=name)
    ...
    def _delete_flow_point(self, name: str) -> None:
        ...
        try:
            self._get_manager().edit_config(
                target=self._get_target(),
                config=Ciena8190.delete_flow_point(name=name))
        except Exception as e:
        ...
        self.log.info(
            'Flow point deleted successfully',
            name=name)

    def _disable_flow_point(self, name: str) -> None:
        """Disable a flow point on the device"""
        self.log.info(
            'Disabling flow point',
            name=name)
        try:
            self._get_manager().edit_config(
                target=self._get_target(),
        ...
        self.log.info(
            'Flow point disabled successfully',
            name=name)
    ...
    def _get_lookup(self) -> dict:
        ...
        if not hasattr(self, '_lookup'):
            ...
            if any([len(v['classifiers']) != 1 for _, v in fps.items()]):
                raise NsiException(GenericRmError, 'Multiple classifiers configured on the single flow point', flow_points=fps)
            ...
            for _, fd in fds.items():
                ...
                values = {
                    ...
                    'classifiers': set([
                        e for fp in fd['flow_points'].values() for e in fp['classifiers']
                    ])
                }
    ...
    def activate(
        ...
        forwarding_domain = 'opennsa-{src_stp}-{dst_stp}-{src_vlan}-{dst_vlan}'.format(
            ...
            src_vlan=src_vlan,
            dst_vlan=dst_vlan)
        src_classifier = 'opennsa-{stp}-{vlan}'.format(
            stp=_src_stp,
            vlan=src_vlan)
        dst_classifier = 'opennsa-{stp}-{vlan}'.format(
            stp=_dst_stp,
            vlan=dst_vlan)
        src_flow_point = 'opennsa-{stp}-{vlan}'.format(
            stp=_src_stp,
            vlan=src_vlan)
        dst_flow_point = 'opennsa-{stp}-{vlan}'.format(
            stp=_dst_stp,
            vlan=dst_vlan)
        ...
        # Create flow points
        self._create_flow_point(name=src_flow_point, fd=forwarding_domain, port=src_port_id, classifier=src_classifier, description=src_flow_point)
        self._create_flow_point(name=dst_flow_point, fd=forwarding_domain, port=dst_port_id, classifier=dst_classifier, description=dst_flow_point)
...
if __name__ == '__main__':
    ...
    def __init__(self):
        ...
        self.topology = lambda: [
            STP(stp_id='AMST1', port_id='19', vlans='1100-4000', description=None, is_alias_in='test.com:2025:production#out', is_alias_out='test.com:2025:production#in', bandwidth=800000, enabled=True, topology='topology'),
            STP(stp_id='NYCN1', port_id='13', vlans='1100-4000', description=None, is_alias_in=None, is_alias_out=None, bandwidth=800000, enabled=True, topology='topology'),
        ]
